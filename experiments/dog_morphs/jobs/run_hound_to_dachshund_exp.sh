#!/bin/bash
#SBATCH --job-name=hound_dachshund_exp
#SBATCH --partition=dev_gpu_h100
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=4
#SBATCH --mem=40G
#SBATCH --time=00:30:00
#SBATCH --output=experiments/dog_morphs/slurm_logs/hound_dachshund_exp_%j.out
#SBATCH --error=experiments/dog_morphs/slurm_logs/hound_dachshund_exp_%j.err
...
CONFIG_FILE="./experiments/dog_morphs/configs/hound_to_dachshund_config.yml"
...
echo "--- Starting Mesh Deformation (Hound -> Dachshund) ---"
python main.py --config "$CONFIG_FILE"
echo "--- Mesh Deformation Finished ---"

# 2) Generate PCA evolution visualization (same style as hippo run)
...
# 4) Keep output clean (same cleanup policy as hound_to_hippo_exp)

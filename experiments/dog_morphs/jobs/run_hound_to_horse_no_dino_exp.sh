#!/bin/bash
#SBATCH --job-name=hound_horse_nodino_exp
#SBATCH --partition=dev_gpu_h100
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=4
#SBATCH --mem=40G
#SBATCH --time=00:30:00
#SBATCH --output=experiments/dog_morphs/slurm_logs/hound_horse_nodino_exp_%j.out
#SBATCH --error=experiments/dog_morphs/slurm_logs/hound_horse_nodino_exp_%j.err
...
CONFIG_FILE="./experiments/dog_morphs/generated_configs/hound_to_horse_no_dino_config_${SLURM_JOB_ID}.yml"

cp "$BASE_CONFIG" "$CONFIG_FILE"

sed -i 's|^output_path:.*|output_path: ./experiments/dog_morphs/outputs/hound_to_horse_no_dino_exp|' "$CONFIG_FILE"
sed -i 's|^text_prompt:.*|text_prompt: "a horse"|' "$CONFIG_FILE"
...
echo "--- Starting Mesh Deformation (Hound -> Horse, NO DINO loss) ---"

#!/bin/bash
#SBATCH --job-name=hound_golden_retriever_nodino_exp
#SBATCH --partition=dev_gpu_h100
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=4
#SBATCH --mem=40G
#SBATCH --time=00:30:00
#SBATCH --output=experiments/dog_morphs/slurm_logs/hound_golden_retriever_nodino_exp_%j.out
#SBATCH --error=experiments/dog_morphs/slurm_logs/hound_golden_retriever_nodino_exp_%j.err
...
CONFIG_FILE="./experiments/dog_morphs/generated_configs/hound_to_golden_retriever_no_dino_config_${SLURM_JOB_ID}.yml"

cp "$BASE_CONFIG" "$CONFIG_FILE"

sed -i 's|^output_path:.*|output_path: ./experiments/dog_morphs/outputs/hound_to_golden_retriever_no_dino_exp|' "$CONFIG_FILE"
sed -i 's|^text_prompt:.*|text_prompt: "a golden retriever"|' "$CONFIG_FILE"
...
echo "--- Starting Mesh Deformation (Hound -> Golden Retriever, NO DINO loss) ---"

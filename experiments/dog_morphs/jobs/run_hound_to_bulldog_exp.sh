#!/bin/bash
#SBATCH --job-name=hound_bulldog_exp
#SBATCH --partition=dev_gpu_h100
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=4
#SBATCH --mem=40G
#SBATCH --time=00:30:00
#SBATCH --output=experiments/dog_morphs/slurm_logs/hound_bulldog_exp_%j.out
#SBATCH --error=experiments/dog_morphs/slurm_logs/hound_bulldog_exp_%j.err
...
CONFIG_FILE="./experiments/dog_morphs/configs/hound_to_bulldog_config.yml"
...
echo "--- Starting Mesh Deformation (Hound -> Bulldog) ---"
...
# 4) Keep output clean (same cleanup policy as dachshund job)

#!/bin/bash
#SBATCH --job-name=h100_hound_hippo_tgt_3st
#SBATCH --partition=gpu_h100
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=120G
#SBATCH --time=10:00:00
#SBATCH --output=jobs_with_sam3D/logs/h100_hound_hippo_tgt_3st_%j.out
#SBATCH --error=jobs_with_sam3D/logs/h100_hound_hippo_tgt_3st_%j.err
...
python main.py \
  ...
  --loss_schedule_stage1_epochs 1000 \
  --loss_schedule_stage2_epochs 2500 \
  --loss_schedule_sds_start 0.10 \
  --loss_schedule_sds_mid 0.45 \
  --loss_schedule_target_floor 0.85 \
  ...
  --elev_max 60 \
  --epochs 3500 \
  --output_path ./jobs_with_sam3D/outputs/hound_to_hippo_bigears_target_three_stage_${SLURM_JOB_ID:-manual}

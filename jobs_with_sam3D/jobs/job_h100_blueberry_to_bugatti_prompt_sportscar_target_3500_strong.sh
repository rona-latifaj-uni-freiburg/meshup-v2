#!/bin/bash
#SBATCH --job-name=h100_blueberry_sport_tgt_strong
#SBATCH --partition=gpu_h100
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=120G
#SBATCH --time=08:00:00
#SBATCH --output=jobs_with_sam3D/logs/h100_blueberry_sport_tgt_strong_%j.out
#SBATCH --error=jobs_with_sam3D/logs/h100_blueberry_sport_tgt_strong_%j.err
...
python main.py \
  ...
  --azim_max 360 \
  --elev_max 60 \
  --output_path ./jobs_with_sam3D/outputs/blueberry_to_bugatti_prompt_sportscar_target_geom_${SLURM_JOB_ID:-manual} \
  --epochs 3500

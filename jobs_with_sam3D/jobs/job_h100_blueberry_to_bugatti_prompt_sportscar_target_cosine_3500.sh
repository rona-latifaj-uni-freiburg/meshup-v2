#!/bin/bash
#SBATCH --job-name=h100_blue_bug_tgt_cos
#SBATCH --partition=gpu_h100
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=120G
#SBATCH --time=08:00:00
#SBATCH --output=jobs_with_sam3D/logs/h100_blue_bug_tgt_cos_%j.out
#SBATCH --error=jobs_with_sam3D/logs/h100_blue_bug_tgt_cos_%j.err
...
python main.py \
  ...
  --loss_schedule_epochs 2500 \
  --loss_schedule_sds_start 0.15 \
  --loss_schedule_target_floor 0.70 \
  ...
  --azim_max 360 \
  --elev_max 60 \
  --output_path ./jobs_with_sam3D/outputs/blueberry_to_bugatti_prompt_sportscar_target_cosine_${SLURM_JOB_ID:-manual} \
  --epochs 3500

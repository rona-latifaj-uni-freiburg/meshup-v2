#!/bin/bash
#SBATCH --job-name=prep_pf_newcar3k
#SBATCH --partition=dev_gpu_h100
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=50G
#SBATCH --time=00:30:00
#SBATCH --output=jobs_with_target_guidance/logs/prep_pf_newcar3k_%j.out
#SBATCH --error=jobs_with_target_guidance/logs/prep_pf_newcar3k_%j.err
...
PARTFIELD_DATA_SUBDIR=${PARTFIELD_DATA_SUBDIR:-data/meshup_new_car_target_runs_5k}
PARTFIELD_RESULT_NAME=${PARTFIELD_RESULT_NAME:-partfield_features/meshup_new_car_target_runs_5k}
PARTFIELD_N_POINT_PER_FACE=${PARTFIELD_N_POINT_PER_FACE:-500}
PARTFIELD_N_SAMPLE_EACH=${PARTFIELD_N_SAMPLE_EACH:-10000}
MESHUP_FEATURE_DIR=${MESHUP_FEATURE_DIR:-${MESHUP_ROOT}/jobs_with_target_guidance/partfield_features/new_car_target_runs_5k}
SEGMENT_OUTPUT_DIR=${SEGMENT_OUTPUT_DIR:-${MESHUP_ROOT}/jobs_with_target_guidance/partfield_segments/new_car_target_runs_5k}
...
MESHES=(
  ...
  "${MESH_DIR}/red_truck1_5k_upright_wheels_down.ply"
  "${MESH_DIR}/red_truck2_5k_upright_wheels_down.ply"
  "${MESH_DIR}/mini_cooper_5k_upright_wheels_down.ply"
...
BASES=(
  f1_car_5k_upright_wheels_down
  f1_verstappen_5k_upright_wheels_down
  red_truck1_5k_upright_wheels_down
  red_truck2_5k_upright_wheels_down
  mini_cooper_5k_upright_wheels_down
  g_class_5k_upright_wheels_down
)

echo "======================================================"
echo "Preparing aligned PartField features for new-car 3k target runs"
...
for bucket_count in "${BUCKETS[@]}"; do
  ...
  for idx in "${!MESHES[@]}"; do
    segment_args+=(
      --mesh "${MESHES[$idx]}"
      --feature "$(feature_path "${BASES[$idx]}")"
      --name "${NAMES[$idx]}"
    )
  done

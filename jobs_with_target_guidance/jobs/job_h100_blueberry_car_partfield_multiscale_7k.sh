#!/bin/bash
#SBATCH --job-name=h100_blue_pfms7k
...
#SBATCH --array=0-1
#SBATCH --output=jobs_with_target_guidance/logs/h100_blue_pfms7k_%A_%a.out
#SBATCH --error=jobs_with_target_guidance/logs/h100_blue_pfms7k_%A_%a.err
...
if [[ "${TASK_ID}" -eq 0 ]]; then
  ...
  TARGET_NAME=bugatti
  LABEL=blueberry_to_bugatti_multiscale_7k
  PROMPT="a sports car"
  ...
  LABEL=blueberry_to_santafe_multiscale_7k
  PROMPT="an SUV"
else
...
echo "======================================================"
echo "H100 blueberry-source multi-scale PartField Chamfer, task ${TASK_ID}, ${EPOCHS} epochs"
echo "SOURCE=${SOURCE}"
echo "PROMPT=${PROMPT}"
echo "TARGET=${TARGET}"

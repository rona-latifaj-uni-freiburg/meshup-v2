#!/bin/bash
#SBATCH --job-name=dev_pit_dach_pf2k
#SBATCH --partition=dev_gpu_h100
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=50G
#SBATCH --time=00:30:00
#SBATCH --output=jobs_with_target_guidance/logs/dev_pit_dach_pf2k_%j.out
#SBATCH --error=jobs_with_target_guidance/logs/dev_pit_dach_pf2k_%j.err
...
LABEL=pitbull_to_dachshund_2k_dev
PROMPT="a dog"

# No literal pitbull mesh is present; hound_to_bulldog is the available dog source proxy.
PARTFIELD_SEGMENT_DIR=${PARTFIELD_SEGMENT_DIR:-./jobs_with_target_guidance/partfield_segments/test_meshes_20}
PARTFIELD_LABEL_DIR=${PARTFIELD_LABEL_DIR:-${PARTFIELD_SEGMENT_DIR}/labels}
SOURCE_PARTFIELD_LABELS=${SOURCE_PARTFIELD_LABELS:-${PARTFIELD_LABEL_DIR}/hound_to_bulldog_partfield_labels.npz}
TARGET_PARTFIELD_LABELS=${TARGET_PARTFIELD_LABELS:-${PARTFIELD_LABEL_DIR}/hound_to_dachshund_partfield_labels.npz}
SOURCE_PARTFIELD_COLORED=${SOURCE_PARTFIELD_COLORED:-${PARTFIELD_SEGMENT_DIR}/colored/hound_to_bulldog_partfield_20_parts.ply}
TARGET_PARTFIELD_COLORED=${TARGET_PARTFIELD_COLORED:-${PARTFIELD_SEGMENT_DIR}/colored/hound_to_dachshund_partfield_20_parts.ply}
...
if [[ ! -f "${SOURCE_PARTFIELD_LABELS}" || ! -f "${TARGET_PARTFIELD_LABELS}" ]]; then
  ...
  echo "Generate them with the test-mesh PartField segmentation workflow."
  exit 1
fi
...
echo "======================================================"
echo "DEV H100 pitbull/bulldog -> dachshund PartField target Chamfer, ${EPOCHS} epochs"
echo "SOURCE=${SOURCE}"
echo "PROMPT=${PROMPT}"
echo "TARGET=${TARGET}"

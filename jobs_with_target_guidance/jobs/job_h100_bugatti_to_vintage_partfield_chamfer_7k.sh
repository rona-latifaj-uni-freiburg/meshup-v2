#!/bin/bash
#SBATCH --job-name=h100_bug_vint_pf7k
#SBATCH --partition=gpu_h100
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=50G
#SBATCH --time=03:00:00
#SBATCH --output=jobs_with_target_guidance/logs/h100_bug_vint_pf7k_%j.out
#SBATCH --error=jobs_with_target_guidance/logs/h100_bug_vint_pf7k_%j.err
...
SOURCE=./jobs_with_sam3D/meshes/5k_upright_wheels_down/bugatti-centodieci_5k_upright_wheels_down.ply
TARGET=./jobs_with_sam3D/meshes/5k_upright_wheels_down/vintage_car_5k_upright_wheels_down.ply
LABEL=bugatti_to_vintage_car_7k
PROMPT="a vintage car"

PARTFIELD_SEGMENT_DIR=${PARTFIELD_SEGMENT_DIR:-./jobs_with_target_guidance/partfield_segments/bugatti_vintage_5k}
PARTFIELD_LABEL_DIR=${PARTFIELD_LABEL_DIR:-${PARTFIELD_SEGMENT_DIR}/labels}
SOURCE_PARTFIELD_LABELS=${SOURCE_PARTFIELD_LABELS:-${PARTFIELD_LABEL_DIR}/bugatti_partfield_labels.npz}
TARGET_PARTFIELD_LABELS=${TARGET_PARTFIELD_LABELS:-${PARTFIELD_LABEL_DIR}/vintage_car_partfield_labels.npz}
SOURCE_PARTFIELD_COLORED=${SOURCE_PARTFIELD_COLORED:-${PARTFIELD_SEGMENT_DIR}/colored/bugatti_partfield_12_parts.ply}
TARGET_PARTFIELD_COLORED=${TARGET_PARTFIELD_COLORED:-${PARTFIELD_SEGMENT_DIR}/colored/vintage_car_partfield_12_parts.ply}

if [[ ! -f "${SOURCE_PARTFIELD_LABELS}" || ! -f "${TARGET_PARTFIELD_LABELS}" ]]; then
  echo "Missing aligned Bugatti/Vintage PartField labels."
...
echo "======================================================"
echo "H100 Bugatti -> Vintage PartField target Chamfer, ${EPOCHS} epochs"
echo "SOURCE=${SOURCE}"
echo "PROMPT=${PROMPT}"
echo "TARGET=${TARGET}"

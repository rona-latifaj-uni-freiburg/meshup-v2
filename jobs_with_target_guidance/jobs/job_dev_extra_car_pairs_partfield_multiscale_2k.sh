#!/bin/bash
#SBATCH --job-name=dev_extra_pfms2k
...
#SBATCH --array=0-1
#SBATCH --output=jobs_with_target_guidance/logs/dev_extra_pfms2k_%A_%a.out
#SBATCH --error=jobs_with_target_guidance/logs/dev_extra_pfms2k_%A_%a.err
...
if [[ "${TASK_ID}" -eq 0 ]]; then
  ...
  TARGET="${NEW_CAR_DIR}/venom_qt_5k_upright_wheels_down.ply"
  SOURCE_NAME=blueberry
  TARGET_NAME=venom_qt
  LABEL=blueberry_to_venom_qt_multiscale_2k_dev
  PROMPT="a sports car"
elif [[ "${TASK_ID}" -eq 1 ]]; then
  SOURCE="${NEW_CAR_DIR}/red_truck1_5k_upright_wheels_down.ply"
  TARGET="${NEW_CAR_DIR}/white_longer_mini_cargo_truck_5k_upright_wheels_down.ply"
  SOURCE_NAME=red_truck1
  TARGET_NAME=white_longer_mini_cargo_truck
  LABEL=red_truck1_to_white_longer_mini_cargo_truck_multiscale_2k_dev
  PROMPT="a white mini cargo truck"
else
...
MULTISCALE_LABEL_DIRS=(
  ./jobs_with_target_guidance/partfield_segments/extra_car_pairs_5k_8
  ./jobs_with_target_guidance/partfield_segments/extra_car_pairs_5k
  ./jobs_with_target_guidance/partfield_segments/extra_car_pairs_5k_20
)
...
for label_dir in "${MULTISCALE_LABEL_DIRS[@]}"; do
  ...
  if [[ ! -f "${source_labels}" || ! -f "${target_labels}" ]]; then
    ...
    echo "  sbatch jobs_with_target_guidance/jobs/job_prepare_partfield_extra_car_pairs_features.sh"
    exit 1
  fi
done
...
echo "======================================================"
echo "DEV extra-car-pairs multi-scale PartField Chamfer, task ${TASK_ID}, ${EPOCHS} epochs"
echo "SOURCE=${SOURCE}"
echo "PROMPT=${PROMPT}"
echo "TARGET=${TARGET}"

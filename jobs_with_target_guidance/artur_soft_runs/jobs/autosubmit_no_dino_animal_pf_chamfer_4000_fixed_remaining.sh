...
LEDGER="${STATE_DIR}/no_dino_animals_4000_fixed_submitted_jobs.tsv"
LOG_FILE="${STATE_DIR}/no_dino_animals_4000_fixed_autosubmit.log"
...
variant_slug() {
  case "$1" in
    pf_chamfer) echo "hard_partfield_chamfer_only" ;;
    pf_chamfer_jneighbor) echo "hard_partfield_chamfer_only_jneighbor${JNEIGHBOR_WEIGHT}" ;;
    *) return 1 ;;
  esac
}
...
output_root() {
  case "$1" in
    pf_chamfer)
      echo "./jobs_with_target_guidance/artur_soft_runs/${OUTPUT_SERIES}/no_regularizer"
      ;;
    pf_chamfer_jneighbor)
      echo "./jobs_with_target_guidance/artur_soft_runs/${OUTPUT_SERIES}/jneighbor_${JNEIGHBOR_WEIGHT}"
      ;;
    *)
      return 1
      ;;
...
submit_opt() {
  ...
  output="$(sbatch \
    --export=ALL,EPOCHS="${EPOCHS}",JNEIGHBOR_WEIGHT="${JNEIGHBOR_WEIGHT}",RUN_TAG="${RUN_TAG}",OUTPUT_SERIES="${OUTPUT_SERIES}" \
  ...
  append_ledger "opt" "${pair_id}" "${pair}" "${mode}" "${job_id}" "-" "${run_dir}"
  return 0
}
...
submit_viz() {
  ...
  rc=$?
  if [[ ${rc} -ne 0 && "${output}" == *"Job dependency problem"* && -f "${run_dir}/correspondence/final_correspondence.json" ]]; then
    log "Dependency is no longer accepted for viz ${pair} ${mode}, but final correspondence exists; submitting viz without dependency."
    output="$(sbatch "${VIZ_JOB}" "${run_dir}" 2>&1)"
    rc=$?
  fi
...
log "Starting fixed no-DINO animal autosubmitter; max_user_jobs=${MAX_USER_JOBS}, poll=${POLL_SECONDS}s"
...
while true; do
  ...
  for mode in pf_chamfer pf_chamfer_jneighbor; do
    for pair_id in 0 1 2 3 4; do
      ...
      if [[ -z "${opt_id}" ]]; then
        missing_any=1
        if submit_opt "${pair_id}" "${mode}"; then
          submitted_any=1
        else
  ...
  if (( missing_any == 0 )); then
    log "All fixed no-DINO animal opt/viz jobs have been submitted."
    exit 0
  fi

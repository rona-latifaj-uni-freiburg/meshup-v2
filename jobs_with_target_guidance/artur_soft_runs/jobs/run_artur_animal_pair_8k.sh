...
MESH_DIR=./external/PartField/data/meshup_test_meshes
PARTFIELD_FEATURE_DIR=./jobs_with_target_guidance/partfield_features/test_meshes
PARTFIELD_LABEL_DIR=./jobs_with_target_guidance/partfield_segments/test_meshes_20/labels
N_BUCKETS=20
JACOBIAN_REG_WEIGHT=${JACOBIAN_REG_WEIGHT:-1000.0}

case "${PAIR_ID}" in
  0)
    PAIR=hound_to_hippo
    SOURCE_NAME=hound_original
    TARGET_NAME=hippo_simple
    PROMPT="a hippo"
    ;;
  1)
    PAIR=hippo_to_hound
    SOURCE_NAME=hippo_simple
    TARGET_NAME=hound_original
    PROMPT="a dog"
    ;;
  2)
    PAIR=hippo_to_bulldog
    SOURCE_NAME=hippo_simple
    TARGET_NAME=hound_to_bulldog
    PROMPT="a bulldog"
    ;;
  3)
    PAIR=hound_to_dachshund
    SOURCE_NAME=hound_original
    TARGET_NAME=hound_to_dachshund
    PROMPT="a dachshund dog"
    ;;
  *)
    echo "Unknown PAIR_ID=${PAIR_ID}. Expected 0, 1, 2, or 3."
    exit 1
    ;;
esac

SOURCE=${MESH_DIR}/${SOURCE_NAME}.obj
TARGET=${MESH_DIR}/${TARGET_NAME}.obj
...
case "${VARIANT_ID}" in
  0)
    VARIANT=hard_partfield_chamfer_reg1000
    PARTFIELD_GUIDANCE_MODE=hard
    PARTFIELD_HARD_WEIGHT=1.0
    PARTFIELD_SOFT_WEIGHT=0.0
    ;;
  1)
    VARIANT=artur_soft_partfield_chamfer_reg1000
    PARTFIELD_GUIDANCE_MODE=soft
    PARTFIELD_HARD_WEIGHT=0.0
    PARTFIELD_SOFT_WEIGHT=1.0
    ;;
  *)
    echo "Unknown VARIANT_ID=${VARIANT_ID}. Expected 0 hard or 1 Artur soft."
    exit 1
    ;;
esac
...
echo "======================================================"
echo "Artur animal PartField Chamfer run"
...
python main.py \
  ...
  --target_mesh_chamfer_weight 0.0 \

#!/usr/bin/env bash
#SBATCH --job-name=sam3d-recon-array
...
if [[ -f "${mesh_out}" ]]; then
  echo "[INFO] Reusing existing mesh for ${IMAGE_NAME}: ${mesh_out}"
  exit 0
fi

if [[ ! -f "${BASE_PATH}/mask/${IMAGE_NAME}" ]]; then
  echo "[ERROR] Missing mask for ${IMAGE_NAME}: ${BASE_PATH}/mask/${IMAGE_NAME}"
  exit 1
fi

export BASE_PATH IMAGE_NAME
bash "${BASE_PATH}/scripts/slurm_sam3d_reconstruct_5k_adaptive.sh"

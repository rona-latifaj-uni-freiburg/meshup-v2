#!/usr/bin/env bash
#SBATCH --job-name=sam3d-recon-adapt
...
echo "[INFO] Attempt 1: high-quality mesh config"
if run_recon "checkpoints/hf/pipeline_mesh_hq.yaml"; then
  echo "[DONE] HQ mesh succeeded for ${IMAGE_NAME}"
  exit 0
fi

echo "[WARN] HQ mesh failed; retrying with robust low-memory config"
run_recon "checkpoints/hf/pipeline_mesh_lowmem.yaml"
echo "[DONE] Low-memory mesh succeeded for ${IMAGE_NAME}"
